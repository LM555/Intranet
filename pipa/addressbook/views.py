
from django.http import HttpResponseRedirect
from django.shortcuts import render_to_response
from django.template import RequestContext
from django.contrib.auth.decorators import login_required
from django.contrib.auth.models import User

from pipa.addressbook.models import PipaProfile
from pipa.addressbook.forms import ProfileForm, NewMemberForm

def alumni(request):
	profiles = PipaProfile.objects.filter(show_profile=True).order_by('user__first_name')
	return render_to_response("alumni/alumni.html", RequestContext(request, locals()))


@login_required
def addressbook(request): 
	profile = request.user.get_profile()

	default_profile_form = ProfileForm(instance=profile, initial={'email': request.user.email, 'first_name': request.user.first_name, 'last_name': request.user.last_name})	
	profile_form = None
	newmemberform = None
	newmemberform_show = False
	
	
	if request.method == 'POST':
		submit_name = ("usersubmit" if 'usersubmit' in request.POST else ("profilesubmit" if 'profilesubmit' in request.POST else None))
		

		profile_form = ProfileForm(request.POST, request.FILES, instance=profile)
		
		if profile_form.is_valid() and submit_name=='profilesubmit':
			# this should probably be better integrated with LDAP
			request.user.email = profile_form.cleaned_data['email']
			request.user.first_name = profile_form.cleaned_data['first_name']
			request.user.last_name = profile_form.cleaned_data['last_name']
			request.user.save()
			profile_form.save()
			return HttpResponseRedirect(request.path)
		
		
		newmemberform = NewMemberForm(request.POST)
		
		if newmemberform.is_valid() and submit_name=='usersubmit':
			# @todo: dejansko dodaj uporabnika v sistem

			newmemberform_show = True
			return render_to_response( # on successful add of new member
                                    "org/addressbook.html",
                         			{'newmember_username': newmemberform.cleaned_data['username'],
									 'profile_form': default_profile_form,
									 'newmemberform': NewMemberForm(),
									 'newmemberform_show': newmemberform_show},
                                    context_instance=RequestContext(request)
                                      )
	
		if submit_name=='usersubmit':
			profile_form = default_profile_form
			newmemberform_show = True
			
		elif submit_name=='profilesubmit':
			newmemberform = NewMemberForm()

	else:
		profile_form = default_profile_form
		newmemberform = NewMemberForm()
		
	
	context = {'profile_form': profile_form,
		'object_list': PipaProfile.objects.all(),
		'user_list': User.objects.all().order_by('username'),
		'newmemberform': newmemberform,
		'newmemberform_show': newmemberform_show
		}
	
	return render_to_response("org/addressbook.html", RequestContext(request, context))
